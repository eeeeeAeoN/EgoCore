...
inline int LZO1X_Decompress(const uint8_t* in, size_t in_len, uint8_t* out, size_t* out_len) {
    const uint8_t* ip = in; const uint8_t* ip_end = in + in_len; uint8_t* op = out; uint8_t* op_start = out;
    *out_len = 0; unsigned int t; unsigned int m_len; const uint8_t* m_pos;
...
LABEL_5:
    t = *ip++;
    if (t < 16) {
        if (t == 0) { while (*ip == 0) { t += 255; ip++; } t += *ip++ + 15; }
        *(uint32_t*)op = *(uint32_t*)ip; op += 4; ip += 4;
        m_len = t;
        if (m_len > 0) {
            m_len--;
            if (m_len < 4) { while (m_len > 0) { *op++ = *ip++; m_len--; } }
            else { do { *(uint32_t*)op = *(uint32_t*)ip; m_len -= 4; op += 4; ip += 4; } while (m_len >= 4); while (m_len > 0) { *op++ = *ip++; m_len--; } }
        }
    first_literal_run:
        t = *ip++;
        if (t < 16) { m_pos = op - (t >> 2) - 2049 - (4 * (*ip++)); *op++ = m_pos[0]; *op++ = m_pos[1]; *op++ = m_pos[2]; goto match_done; }
    }

    while (true) {
        if (t >= 64) { m_pos = op - 1 - ((t >> 2) & 7) - (8 * (*ip++)); m_len = (t >> 5) - 1; goto copy_match; }
        if (t >= 32) {
            m_len = t & 31;
            if (m_len == 0) { while (*ip == 0) { m_len += 255; ip++; } m_len += *ip++ + 31; }
            m_pos = op - 1 - (*(uint16_t*)ip >> 2); ip += 2;
            goto do_match;
        }
        if (t >= 16) {
            m_pos = op - 0x800 * (t & 8); m_len = t & 7;
            if (m_len == 0) { while (*ip == 0) { m_len += 255; ip++; } m_len += *ip++ + 7; }
            uint16_t offset = *(uint16_t*)ip; ip += 2; m_pos -= (offset >> 2);
            if (m_pos == op) { *out_len = op - op_start; if (ip == ip_end) return 0; return (ip < ip_end) ? -8 : -4; }
            m_pos -= 0x4000;
            goto do_match;
        }
        m_pos = op - 1 - (t >> 2) - (4 * (*ip++)); *op++ = m_pos[0]; *op++ = m_pos[1];

    match_done:
        t = *(ip - 2) & 3; if (t == 0) goto LABEL_5;
...
do_match:
    if (m_len >= 6 && (op - m_pos) >= 4) {
        *(uint32_t*)op = *(uint32_t*)m_pos; op += 4; m_pos += 4;
        unsigned int remaining = m_len - 2;
        do { *(uint32_t*)op = *(uint32_t*)m_pos; remaining -= 4; op += 4; m_pos += 4; } while (remaining >= 4);
...
inline std::vector<uint8_t> DecompressLZO(const uint8_t* data, size_t& cursor, size_t fileSize, size_t expectedSize) {
    ...
    while (totalDecompressed < lzoTarget && cursor + 2 <= fileSize) {
        uint32_t compSize = *(uint16_t*)(data + cursor); cursor += 2;
        if (compSize == 0xFFFF) { if (cursor + 4 > fileSize) break; compSize = *(uint32_t*)(data + cursor); cursor += 4; }
        if (compSize == 0) {
            ...
            size_t outLen = 0;
            int ret = LZO1X_Decompress(data + cursor, compSize, result.data() + totalDecompressed, &outLen);
...
inline std::vector<uint8_t> DecompressRawLZO(const std::vector<uint8_t>& data, size_t offset, size_t expectedSize) {
    ...
    int ret = LZO1X_Decompress(data.data() + offset, data.size() - offset, result.data(), &processedSize);
