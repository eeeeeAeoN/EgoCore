...
static void DrawAudioProperties(LoadedBank* bank) {
    ...
    // --- BUTTONS ---
    // Save (Green if modified)
    bool hasAnyMods = !bank->AudioParser->ModifiedCache.empty();
    if (hasAnyMods) {
        ImGui::PushStyleColor(ImGuiCol_Button, ImVec4(0.0f, 0.7f, 0.0f, 1.0f));
        if (ImGui::Button("SAVE BANK", ImVec2(-1, 40))) {
            // Recompile to original file
            bank->AudioParser->SaveBank(bank->FullPath);
        }
        ImGui::PopStyleColor();
        ImGui::Dummy(ImVec2(0, 5));
    }
    ...
    if (ImGui::Button("Import", ImVec2(buttonWidth, 40))) {
