...
inline void CreateNewTextureEntry(LoadedBank* bank, const std::string& filePath, ETextureFormat fmt = ETextureFormat::DXT3, int entryType = 0) {
    ...
    newEntry.Type = entryType; // Apply requested type
...
inline void AddTextureFrame(LoadedBank* bank, int entryIdx, const std::string& filePath) {
    ...
    // [FIX] Auto-Convert Type when adding frames
    int& type = bank->Entries[entryIdx].Type;
    if (type == 0 && header->FrameCount > 1) type = 1;      // Graphic -> Graphic Seq
    else if (type == 2 && header->FrameCount > 1) type = 3; // Bumpmap -> Bumpmap Seq
...
inline void DeleteTextureFrame(LoadedBank* bank, int entryIdx, int frameIdx) {
    ...
    if (offset + stride <= currentData.size()) {
        ...
        // [FIX] Auto-Convert Type back to Single if it drops to 1 frame
        int& type = bank->Entries[entryIdx].Type;
        if (type == 1 && header->FrameCount <= 1) type = 0;      // Graphic Seq -> Graphic
        else if (type == 3 && header->FrameCount <= 1) type = 2; // Bumpmap Seq -> Bumpmap
