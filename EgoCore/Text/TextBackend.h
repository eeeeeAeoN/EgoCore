...
inline std::string GetHeaderForSubBank(const std::string& subBankName) {
    ...
    if (upper.find("LIPSYNC_") == 0) {
        ...
        if (upper.find("_SCRIPT_2") != std::string::npos) return "scriptdialoguesnds2.h";
        if (upper.find("_SCRIPT") != std::string::npos) return "scriptdialoguesnds.h";
    }
    return "";
}
...
inline std::shared_ptr<AudioBankParser> GetOrLoadAudioBank(const std::string& bankName) {
    ...
    if (lugPos != std::string::npos) searchKey.replace(lugPos, 4, ".lut");
    else if (searchKey.find(".") == std::string::npos) searchKey += ".lut";
