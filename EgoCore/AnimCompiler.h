...
public:
    static std::vector<uint8_t> Compile(const C3DAnimationInfo& anim) {
        ...
        if (hasGlobalIDs) {
            std::vector<AnimTrack> sortedTracks;
            std::vector<bool> inserted(safeTracks.size(), false);
            for (size_t i = 0; i < safeTracks.size(); i++) {
                if (safeTracks[i].ParentIndex == -1) { sortedTracks.push_back(safeTracks[i]); inserted[i] = true; }
            }
            bool progress = true;
            while (progress) {
                progress = false;
                for (size_t i = 0; i < safeTracks.size(); i++) {
                    if (!inserted[i]) {
                        ...
                        for (const auto& st : sortedTracks) {
                            if (st.BoneIndex == safeTracks[i].ParentIndex) { parentFound = true; break; }
                        }
                        if (parentFound) { sortedTracks.push_back(safeTracks[i]); inserted[i] = true; progress = true; }
                    }
                }
            }
            for (size_t i = 0; i < safeTracks.size(); i++) {
                if (!inserted[i]) { safeTracks[i].ParentIndex = -1; sortedTracks.push_back(safeTracks[i]); }
            }
            safeTracks = sortedTracks;
            for (size_t i = 0; i < safeTracks.size(); i++) {
                int targetParentFableID = safeTracks[i].ParentIndex;
                int newParentIndex = -1;
                if (targetParentFableID != -1) {
                    for (size_t j = 0; j < i; j++) {
                        if (safeTracks[j].BoneIndex == targetParentFableID) { newParentIndex = (int)j; break; }
                    }
                }
        ...
        if (actualDuration <= 0.0f) actualDuration = anim.Duration;

        WriteChunk(w, "ANRT", [&](AnimWriter& sw) {
            sw.Write<uint8_t>(anim.IsCyclic ? 1 : 0);
            sw.Write<float>(actualDuration);
            ...
            WriteChunk(sw, "XALO", [&](AnimWriter& xaloW) {
                ...
                for (const auto& t : safeHelperTracks) allocSize += calcTrackSize(t);
                xaloW.Write<uint32_t>(allocSize);
                });

            bool needsHelper = anim.HasHelper || !safeHelperTracks.empty() || !anim.TimeEvents.empty() ||
            ...
            if (needsHelper) {
                WriteChunk(sw, "HLPR", [&](AnimWriter& hW) {
                    WriteChunk(hW, "MVEC", [&](AnimWriter& vecW) {
                        ...
                        for (const auto& track : safeHelperTracks) {
                            if (track.BoneName == "") { WriteChunk(vecW, "XSEQ", [&](AnimWriter& seqW) { WriteTrackPayload(seqW, track); }); break; }
                        }
                        });
                    for (const auto& ev : anim.TimeEvents) {
                        WriteChunk(hW, "TMEV", [&](AnimWriter& tW) { tW.WriteString(ev.Name); tW.Write<float>(ev.Time); });
                    }
                    for (const auto& track : safeHelperTracks) {
                        if (track.BoneName != "") WriteChunk(hW, "XSEQ", [&](AnimWriter& seqW) { WriteTrackPayload(seqW, track); });
                    }
                    });
            }
            ...
            WriteChunk(sw, "AOBJ", [&](AnimWriter& objW) {
                ...
                for (const auto& track : safeTracks) WriteChunk(objW, "XSEQ", [&](AnimWriter& seqW) { WriteTrackPayload(seqW, track); });
                });
            });
        ...
        memcpy(finalBlob.data(), &uncompLen, 4); memcpy(finalBlob.data() + 4, compBuf.data(), compLen);
        return finalBlob;
    }

private:
    struct PosPaletteResult { std::vector<uint16_t> Indices; std::vector<int16_t> CompressedPalette; float Factor = 1.0f; };
    struct RotPaletteResult { std::vector<uint16_t> Indices; std::vector<Vec4> Palette; };

    static PosPaletteResult ProcessPositions(const AnimTrack& t) {
        PosPaletteResult res;
        if (t.BoneName == "Scene Root" || t.BoneName == "Movement" || t.BoneName == "Movement_dummy" || t.BoneName == "Sub_movement_dummy") return res;
        ...
        if (!t.PalettedPositions.empty()) {
            ...
            for (size_t i = 1; i < t.PositionTrack.size(); i++) {
                if (std::abs(t.PositionTrack[i].x - t.PositionTrack[0].x) > 0.001f || std::abs(t.PositionTrack[i].y - t.PositionTrack[0].y) > 0.001f || std::abs(t.PositionTrack[i].z - t.PositionTrack[0].z) > 0.001f) { isConstant = false; break; }
            }
            if (isConstant) {
    ...
    static RotPaletteResult ProcessRotations(const AnimTrack& t) {
        ...
        if (t.BoneName == "Scene Root" || t.BoneName == "Movement" || t.BoneName == "Movement_dummy" || t.BoneName == "Sub_movement_dummy") return res;
        if (!t.PalettedRotations.empty()) { res.Indices = t.PalettedRotations; res.Palette = t.RotationTrack; }
        else if (!t.RotationTrack.empty()) {
            bool isConstant = true;
            for (size_t i = 1; i < t.RotationTrack.size(); i++) {
                if (std::abs(t.RotationTrack[i].x - t.RotationTrack[0].x) > 0.0001f || std::abs(t.RotationTrack[i].y - t.RotationTrack[0].y) > 0.0001f || std::abs(t.RotationTrack[i].z - t.RotationTrack[0].z) > 0.0001f || std::abs(t.RotationTrack[i].w - t.RotationTrack[0].w) > 0.0001f) { isConstant = false; break; }
            }
            if (isConstant) {
                auto r = t.RotationTrack[0];
                if (!(std::abs(r.x) < 0.001f && std::abs(r.y) < 0.001f && std::abs(r.z) < 0.001f && std::abs(std::abs(r.w) - 1.0f) < 0.001f) && !(std::abs(std::abs(r.x) - 1.0f) < 0.001f && std::abs(r.y) < 0.001f && std::abs(r.z) < 0.001f && std::abs(r.w) < 0.001f)) res.Palette.push_back(r);
            }
            else { res.Palette = t.RotationTrack; for (size_t i = 0; i < t.RotationTrack.size(); i++) res.Indices.push_back((uint16_t)i); }
        }
        return res;
    }

    static void WriteChunk(AnimWriter& w, const char* id, std::function<void(AnimWriter&)> content) {
        w.Write(id, 4); size_t sizePos = w.GetPos(); w.Write<uint32_t>(0);
        size_t start = w.GetPos(); content(w);
        uint32_t size = (uint32_t)(w.GetPos() - start); w.WriteAt(sizePos, &size, 4);
    }

    static void WriteTrackPayload(AnimWriter& w, const AnimTrack& t) {
        w.Write<uint32_t>(t.BoneIndex); w.Write<int32_t>(t.ParentIndex); w.WriteString(t.BoneName);
        w.Write<uint8_t>(0); w.Write<float>(t.SamplesPerSecond); w.Write<uint32_t>(t.FrameCount);
        auto posDat = ProcessPositions(t); auto rotDat = ProcessRotations(t);
        ...
        flags[0] = (flags[1] > 0 || flags[2] > 0) ? 1 : 0; flags[3] = 0;
        w.Write(flags, 4);
        w.Write<float>(posDat.Factor); w.Write<float>(t.ScalingFactor == 0.0f ? 1.0f : t.ScalingFactor);
